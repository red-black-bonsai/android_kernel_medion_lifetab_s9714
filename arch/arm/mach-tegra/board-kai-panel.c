...
#define kai_a2110_bl_enb			TEGRA_GPIO_PH7 /* 1.EN_VDD_BL1 */ /*20120822 JimmySu add for A2110 10" panel*/
...
/*20120822 JimmySu add for A2110 10" panel*/
	ret = gpio_request(kai_a2110_bl_enb, "backlight_enb");
	if (ret < 0)
		return ret;

	ret = gpio_direction_output(kai_a2110_bl_enb, 1);
	if (ret < 0)
		gpio_free(kai_a2110_bl_enb);
	else
		tegra_gpio_enable(kai_a2110_bl_enb);
/*20120822 JimmySu add for A2110 10" panel*/
	return ret;
};
...
static void kai_backlight_exit(struct device *dev)
{
	...
	gpio_set_value(kai_a2110_bl_enb, 0);  /*20120822 JimmySu add for A2110 10" panel*/
...
//	gpio_set_value(kai_vdd_bl_enb, !!brightness); /*20120607, JimmySu remove unsed setting*/
	if(cl2n_panel_status == 1)
		gpio_set_value(kai_a2110_bl_enb, !!brightness); /*20120822 JimmySu add for A2110 10" panel*/
//	else
//		gpio_set_value(kai_a2110_bl_enb, 0); /*20121130, JimmySu remove useless function*/ 
...
// panel default setting
	...
	if (panel_board_id == 0x7){ //A2109 EVT2 
...
//5. Reset ----\___/----
	...
	gpio_set_value(kai_a2110_bl_enb, 1); /*20120822 JimmySu add for A2110 10" panel*/
	mdelay(11);
...
/*----20120608, JimmySu prevent flash screen while shutdown device*/
//{CL2N}-
	mdelay(10);  /*20120822 JimmySu add for A2110 10" panel timing*/

	gpio_set_value(kai_vdd_bl_enb, 0);
	gpio_set_value(kai_a2110_bl_enb, 0);  /*20120822 JimmySu add for A2110 10" panel*/
	
	udelay(20);
	if (panel_board_id == 0x7){ //A2109 EVT
...
//	mdelay(100);
	...
	mdelay(200);  /*20120822 JimmySu modify for A2110 10" panel timing*/
...
/*20121023, JimmySu add for pass CTS*/
	.height = 136, /* mm */
	.width = 217, /* mm */
/*20121023, JimmySu add for pass CTS*/
