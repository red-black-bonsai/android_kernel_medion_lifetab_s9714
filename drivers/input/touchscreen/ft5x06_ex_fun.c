...
static unsigned char CTPM_FW_0x22[] = {
	#include "A2110_FT5606_Ver0x22_app.i"
};

static unsigned char CTPM_FW_0x34[] = {
	#include "A2110_FT5606_Ver0x34_app.i"
};
...
int fts_ctpm_fw_upgrade_with_i_file_A2110(struct i2c_client *client, u8 uc_tp_fm_ver)
{
	u8 *pbt_buf = NULL;
	int i_ret;
	int fw_len;
	...
	*/
	...
	if (uc_tp_fm_ver == 0x21)
		fw_len = sizeof(CTPM_FW_0x22);
	else
		fw_len = sizeof(CTPM_FW_0x34);
	...
	if (uc_tp_fm_ver == 0x21){
		...
		if ((CTPM_FW_0x22[fw_len - 8] ^ CTPM_FW_0x22[fw_len - 6]) == 0xFF
			&& (CTPM_FW_0x22[fw_len - 7] ^ CTPM_FW_0x22[fw_len - 5]) == 0xFF
			&& (CTPM_FW_0x22[fw_len - 3] ^ CTPM_FW_0x22[fw_len - 4]) == 0xFF) {
			/*FW upgrade */
			pbt_buf = CTPM_FW_0x22;
			/*call the upgrade function */
			i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_0x22));
			if (i_ret != 0)
				dev_err(&client->dev, "%s:upgrade failed. err.\n",
						__func__);
		...
		if ((CTPM_FW_0x34[fw_len - 8] ^ CTPM_FW_0x34[fw_len - 6]) == 0xFF
			&& (CTPM_FW_0x34[fw_len - 7] ^ CTPM_FW_0x34[fw_len - 5]) == 0xFF
			&& (CTPM_FW_0x34[fw_len - 3] ^ CTPM_FW_0x34[fw_len - 4]) == 0xFF) {
			/*FW upgrade */
			pbt_buf = CTPM_FW_0x34;
			/*call the upgrade function */
			i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_0x34));
			if (i_ret != 0)
				dev_err(&client->dev, "%s:upgrade failed. err.\n",
						__func__);
...
u8 fts_ctpm_get_22_i_file_ver(void)
{
	u16 ui_sz;
	ui_sz = sizeof(CTPM_FW_0x22);
	if (ui_sz > 2)
		return CTPM_FW_0x22[ui_sz - 2];

	return 0x00;	/*default value */
}

u8 fts_ctpm_get_34_i_file_ver(void)
{
	u16 ui_sz;
	ui_sz = sizeof(CTPM_FW_0x34);
	if (ui_sz > 2)
		return CTPM_FW_0x34[ui_sz - 2];
...
//	uc_host_fm_ver = fts_ctpm_get_i_file_ver();
	if(uc_tp_fm_ver == 0x21)
		uc_host_fm_ver = fts_ctpm_get_22_i_file_ver();
	else
		uc_host_fm_ver = fts_ctpm_get_34_i_file_ver();
...
//		i_ret = fts_ctpm_fw_upgrade_with_i_file(client);
		i_ret = fts_ctpm_fw_upgrade_with_i_file_A2110(client, uc_tp_fm_ver);
