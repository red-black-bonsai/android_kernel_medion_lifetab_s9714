...
static void ft5x0x_ts_resume(struct early_suspend *handler)
{
	...
	struct ft5x0x_ts_data *ts = container_of(handler, struct ft5x0x_ts_data,
						early_suspend);
/**++++20121214, JimmySu disable idle mode, touch is always in active mode**/
	u8 pack_buf[2];
/**-----20121214, JimmySu disable idle mode, touch is always in active mode**/
...
/**++++20121214, JimmySu disable idle mode, touch is always in active mode**/
	pack_buf[0] = 0x86;
	pack_buf[1] = 0x0;
	ft5x0x_i2c_Write(ts->client, pack_buf, 2);
/**-----20121214, JimmySu disable idle mode, touch is always in active mode**/
...
//		pr_info("%s++\n", __func__);
	...
	mdelay(1); /*20121130, JimmySu add for make sure reset -> 1ms delay -> VDD off*/
...
/*20121122, JimmySu add upgrade touch FW*/
	uc_reg_addr = FT5x0x_REG_FW_VER;
	ft5x0x_i2c_Read(client, &uc_reg_addr, 1, &uc_reg_value, 1);
	if ((uc_reg_value == 0x21 ) || (uc_reg_value == 0x33 ) || (uc_reg_value == 0xA6 ))
	{
...
/**++++20121214, JimmySu disable idle mode, touch is always in active mode**/
	pack_buf[0] = 0x86;
	pack_buf[1] = 0x0;
	ft5x0x_i2c_Write(client, pack_buf, 2);
/**----20121214, JimmySu disable idle mode, touch is always in active mode**/
