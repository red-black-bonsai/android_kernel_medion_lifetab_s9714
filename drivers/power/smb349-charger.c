...
static int smb349_configure_otg(struct i2c_client *client, int enable)
{
	...
	if (enable) {
		...
		ret = smb349_write(client, SMB349_OTG_TLIM_REG, 0x4C/*(ret | 0x0F)*/);
		if (ret < 0) {			
			dev_err(&client->dev, "%s: err %d\n", __func__, ret);
		}
...
static int smb349_enable_otg(struct regulator_dev *otg_rdev){
	...
	#if 1 //To distinguish OTG and AC/USB cable
	...
	printk("[smb349-charger.c]%s(%d) enable OTG\n", __func__, __LINE__);
...
static int smb349_disable_otg(struct regulator_dev *otg_rdev){	
	...
	printk("[smb349-charger.c]%s(%d)disable OTG\n", __func__, __LINE__);
